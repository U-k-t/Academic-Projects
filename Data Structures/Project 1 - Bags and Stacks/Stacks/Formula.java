public class Formula
{
   ...
   public Formula(String expression) //Creates an Expression Equal to the Inputted String
   {
      usrExpression = expression;
   }
   public Formula () //Creates an Empty Expression
   {
      this("");
   }
   ...
   public String toPreFix() // Converts Infix Expression to Prefix
   {
      ...
      ArrayStack reversedStack = new ArrayStack();
      int counter = 0;
      for (int i = 0; i<getExpression().length(); i++)
      {
   ...
   {
      ArrayStack parenStack = new ArrayStack(getExpression().length());
